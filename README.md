...
| `Stop` | **Save reminder**: counts user messages in the transcript. First reminder at 20 messages, then every 20 more (40, 60, 80...). Shows `systemMessage` suggesting `/recall-save`. Non-blocking, per-session state. |
...
2. **If a session already exists for the same day**, reads the existing JSON and performs an intelligent merge (see below)
3. Saves the consolidated JSON to `~/.claude/memory/project_date.json`
4. Chunks the merged summary and re-indexes embeddings locally in SQLite (atomic transaction)
5. Rotates sessions if needed (max 3 active)

#### Intelligent merge (v2.1)

When you run `/recall-save` multiple times in the same day, the plugin no longer overwrites the previous session. Instead, it merges the data intelligently:

| Field | Merge strategy |
|-------|---------------|
| `decisions`, `concepts`, `files_modified` | Union + deduplicate (new items appended after existing) |
| `tasks_completed` | Union + deduplicate |
| `tasks_pending` | Union + deduplicate, then **remove items that appear in `tasks_completed`** |
| `title`, `notes` | New replaces existing (latest context wins) |

This means a session accumulates knowledge throughout the day:
- Morning save: 3 decisions, 2 tasks pending
- Afternoon save: 1 new decision + 1 task completed (was pending)
- Result: 4 decisions, 1 task pending, 3 tasks completed

The merge happens in Python before writing to SQLite. The re-indexation of chunks (DELETE old + INSERT new in sqlite-vec and FTS5) runs inside a single `BEGIN IMMEDIATE` transaction, guaranteeing atomicity — the `UserPromptSubmit` hook never sees partial state.
...
|---------|-------|----------|
| ~~Multiple saves per day overwrite previous session~~ | ~~Same `session_id` (`project_date`) caused full overwrite~~ | **Fixed in v2.1** — intelligent merge (union + dedup arrays, atomic re-indexation) |
